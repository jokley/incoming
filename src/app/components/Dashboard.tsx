...
export function Dashboard() {
  ...
  const criticalAlerts = useMemo<AlertItem[]>(() => {
    ...
    return [
      ...(operations.peopleWithoutRoom > 0 ? [{ id: 'open-assignments', title: 'Personen ohne Zimmer', detail: `${operations.peopleWithoutRoom} Personen sind noch keiner Unterkunft zugewiesen.`, tone: 'error' as const, status: 'Sofort', href: '/lists?entity=persons&hint=without-room' }] : []),
      ...
      <div className="grid grid-cols-1 gap-2 xl:grid-cols-[1fr_1.02fr]">
        <DataPanel title={<span className="inline-flex items-center gap-2"><WarningAmberRoundedIcon fontSize="small" />Entscheidungen</span>}>
          <div className="grid grid-cols-1 gap-2 p-2 md:grid-cols-2 xl:grid-cols-3">
            {criticalAlerts.length === 0 ? <ContentCard className="col-span-full flex items-center justify-center gap-3 p-4" surface="elevated" elevation="none"><IconTile tone="success" icon={<CheckRoundedIcon />} /><strong className="text-sm text-[var(--ops-success)]">Keine offenen Entscheidungen</strong></ContentCard> : criticalAlerts.map(alert => <ContentCard key={alert.id} className="p-3" surface="elevated" elevation="none"><div className="flex flex-wrap items-start gap-2"><div className="flex min-w-0 flex-[1_1_12rem] items-start gap-3"><IconTile tone={alert.tone} icon={alert.tone === 'warning' ? <ShieldRoundedIcon /> : <WarningAmberRoundedIcon />} /><h3 className="min-w-0 break-words text-sm font-extrabold uppercase leading-5 text-[var(--ops-text)]">{alert.title}</h3></div><div className="shrink-0 whitespace-nowrap"><StatusChip tone={alert.tone}>{alert.status}</StatusChip></div></div><p className="mt-2 text-xs leading-5 text-[var(--ops-text-muted)]">{alert.detail}</p><div className="mt-2"><TextLink to={alert.href}>Details anzeigen</TextLink></div></ContentCard>)}
          </div>
        </DataPanel>
