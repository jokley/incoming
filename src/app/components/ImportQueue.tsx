...
const completedStatuses = new Set<ImportSessionStatus>(['IMPORTIERT', 'ERSETZT', 'ABGEBROCHEN']);

export function ImportQueue({ selectedId, isCreating, onCreate, onSelect }: { selectedId: string | null; isCreating: boolean; onCreate: () => void; onSelect: (session: ImportSession) => void }) {
  ...
  const openSessions = filtered.filter(session => !completedStatuses.has(session.status));
  const completedSessions = filtered.filter(session => completedStatuses.has(session.status));
