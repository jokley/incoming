...
function StatPill({ label, value, icon, emphasis = false }: { label: string; value: number | string; icon: React.ReactNode; emphasis?: boolean }) {
  return <div className={clsx('flex min-h-[5.75rem] flex-col justify-between rounded-[var(--ops-radius-lg)] border p-4', emphasis ? 'border-[var(--ops-tone-success-border)] bg-[var(--ops-tone-success-surface)]' : 'border-transparent bg-[var(--ops-surface-raised)]')}><div className={clsx('flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.08em]', emphasis ? 'text-[var(--ops-tone-success-text)]' : 'text-[var(--ops-text-subtle)]')}>{icon}{label}</div><div className={clsx('mt-2 font-extrabold leading-none text-[var(--ops-text)]', emphasis ? 'text-3xl' : 'text-xl')}>{value}</div></div>;
}
...
export function HotelsManagement() {
  ...
  const handleCancelInventory = () => { setShowInventoryForm(false); setInventoryForm({ roomTypeId: '', availableFrom: '', availableUntil: '', roomCount: 0, hasHalfBoard: false, hasSR: false }); };
  ...
  return <PageLayout className="[--ops-background:#111d2e] [--ops-surface:#1a2a40] [--ops-surface-raised:#21334c] [--ops-surface-elevated:#2a3e59] [--ops-surface-overlay:#344b67] [--ops-border:#4b6380] [--ops-border-strong:#6d86a5] [--ops-divider:#405773] [--ops-text-muted:#b7c4d4] [&_h1]:text-[var(--ops-text)] [&_h2]:text-[var(--ops-text)] [&_h3]:text-[var(--ops-text)] [&_h4]:text-[var(--ops-text)]">
    <PageHeader
    ...
    <div className="grid h-[calc(100vh-19rem)] min-h-[34rem] grid-cols-1 gap-5 overflow-hidden xl:grid-cols-[minmax(21rem,0.34fr)_minmax(0,0.66fr)]">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-3">
            <div className="space-y-2">
              {filteredHotels.map((hotel) => {
                ...
                return <button key={hotel.id} onClick={() => setSelectedHotelId(hotel.id)} className={clsx('w-full rounded-[var(--ops-radius-lg)] border border-l-4 px-3 py-2.5 text-left transition-colors hover:border-[var(--ops-primary)] hover:bg-[var(--ops-surface-elevated)] focus-visible:outline-none focus-visible:shadow-[var(--ops-focus-ring)]', active ? 'border-[var(--ops-primary)] bg-[var(--ops-surface-overlay)] shadow-[var(--ops-shadow-sm)]' : 'border-[var(--ops-border)] border-l-transparent bg-[var(--ops-surface)]')} aria-pressed={active}>
      ...
      <ContentCard className="flex min-h-0 overflow-hidden" surface="raised">
        <div className="flex min-h-0 w-full flex-col">
          ...
          <div className="overflow-hidden rounded-[var(--ops-radius-xl)] border border-[var(--ops-border)] bg-[var(--ops-surface)]">
            <div className="grid grid-cols-[minmax(9rem,0.34fr)_1fr] border-b border-[var(--ops-divider)] bg-[var(--ops-surface-raised)] px-4 py-2 text-[10px] font-bold uppercase tracking-[0.1em] text-[var(--ops-text-subtle)]"><span>Zimmerkategorie</span><div className="flex justify-between"><span>Start</span><span>Auslastung im Zeitraum</span><span>Ende</span></div></div>
            <div className="divide-y divide-[var(--ops-divider)]">{(selectedHotel.roomInventories || []).map((inventory) => { const invStats = getInventoryStats(inventory, bookings.filter((booking) => booking.hotel.id === selectedHotel.id)); return <div key={inventory.id} className="grid grid-cols-[minmax(9rem,0.34fr)_1fr] items-center gap-4 px-4 py-3 hover:bg-[var(--ops-surface-raised)]"><div><div className="font-bold text-[var(--ops-text)]">{inventory.roomType.name}</div><div className="mt-1 text-xs text-[var(--ops-text-muted)]">{inventory.roomCount} Zimmer · {invStats.freeRooms} frei</div></div><div><div className="mb-2 flex items-center justify-between gap-3 text-xs"><time className="font-semibold text-[var(--ops-text)]">{formatDate(inventory.availableFrom)}</time><StatusChip tone={invStats.tone}>{formatPercent(invStats.occupancy)} belegt</StatusChip><time className="font-semibold text-[var(--ops-text)]">{formatDate(inventory.availableUntil)}</time></div><div className="relative"><ProgressBar value={invStats.occupancy} tone={invStats.tone} /><span className="absolute left-1/2 top-0 h-2 w-px bg-[var(--ops-border-strong)]" /></div></div></div>; })}</div>
          </div>
          <div className="flex flex-wrap items-center justify-end gap-2 border-t border-[var(--ops-divider)] pt-4">
            <Link to={assignmentHref(selectedHotel.id)} className="rounded-[var(--ops-radius-md)] border border-[var(--ops-border)] bg-[var(--ops-tone-primary-surface)] px-3 py-2 text-sm font-bold text-[var(--ops-tone-primary-text)]">Zimmerbelegung öffnen <ArrowRight className="inline h-4 w-4" /></Link>
            <OpsButton onClick={() => handleEdit(selectedHotel)} disabled={!permissions.canEdit} title={!permissions.canEdit ? READ_ONLY_TOOLTIP : undefined}><Pencil className="mr-2 inline h-4 w-4" />Hotel bearbeiten</OpsButton>
            <OpsButton onClick={() => setShowInventoryForm(true)} disabled={!permissions.canCreate}><Plus className="mr-2 inline h-4 w-4" />Kontingent erstellen</OpsButton>
          </div>
