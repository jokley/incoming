...
export function EventsManagement() {
  ...
  const timelineRows: TimelineRowData[] = events.map(event => {
    const stats = demandStats(event);
    return { id: event.id, title: competitionDisplayName(event.discipline), subtitle: `${inclusiveDays(event)} Tage`, segments: [{ id: `event-${event.id}`, start: event.startDate, end: event.endDate, label: competitionDisplayName(event.discipline), color: event.id === selectedId ? 'var(--ops-primary-emphasis)' : 'var(--ops-info)', tooltipData: { title: competitionDisplayName(event.discipline), description: <><div>Personenbedarf: {stats.people}</div><div>Berechnete Zimmer: {stats.rooms}</div><div>EZ-Anteil: {stats.singlePercentage} %</div><div>DZ-Anteil: {stats.doublePercentage} %</div></> } }] };
  });
  ...
  return <SplitPageLayout>
    ...
    <div className="flex flex-col gap-4 xl:min-h-0 xl:flex-1 xl:flex-row">
      <ContentCard surface="raised" className="flex flex-col overflow-hidden xl:min-h-0 xl:w-[22rem] xl:shrink-0"><div className="shrink-0 border-b border-[var(--ops-divider)] p-4"><SectionHeader title={`Eventliste (${filtered.length})`} /><label className="mt-3 flex items-center gap-2 rounded-lg border border-[var(--ops-border)] bg-[var(--ops-surface-elevated)] px-3 py-2"><Search className="h-4 w-4" /><input aria-label="Events suchen" className="w-full bg-transparent text-sm outline-none" placeholder="Event oder Disziplin suchen" value={search} onChange={e => setSearch(e.target.value)} /></label></div>
        <div className="space-y-2 p-3 xl:min-h-0 xl:flex-1 xl:overflow-y-auto">{filtered.map(event => { const stats = demandStats(event); const status = eventStatus(event); return <button key={event.id} onClick={() => setSelectedId(event.id)} className={clsx('w-full rounded-xl border p-3 text-left transition hover:bg-[var(--ops-surface-elevated)]', selectedId === event.id ? 'border-[var(--ops-primary)] bg-[var(--ops-surface-overlay)]' : 'border-[var(--ops-border)] bg-[var(--ops-surface)]')}><div className="flex justify-between gap-2"><div><b>{competitionDisplayName(event.discipline)}</b><div className="text-xs text-[var(--ops-text-muted)]">{formatDate(event.startDate)} – {formatDate(event.endDate)}</div><div className="text-xs text-[var(--ops-text-muted)]">{inclusiveDays(event)} Tage</div></div>{status && <StatusChip tone={status.tone}>{status.label}</StatusChip>}</div><div className="mt-2 text-xs text-[var(--ops-text-muted)]">{stats.people} Personenbedarf · {stats.rooms} Zimmer berechnet</div></button>; })}{!filtered.length && <EmptyState title="Keine Events gefunden" />}</div>
      </ContentCard>
      <ContentCard surface="raised" className="flex min-h-0 flex-1 flex-col overflow-hidden">{selected ? <><div className="flex shrink-0 items-start justify-between gap-4 border-b border-[var(--ops-divider)] p-5"><div><SectionHeader title="Eventinformationen" /><h2 className="mt-3 text-2xl font-extrabold">{competitionDisplayName(selected.discipline)}</h2><div className="mt-1 flex items-start gap-2 text-sm text-[var(--ops-text-muted)]"><CalendarDays className="mt-0.5" size={16} /><div><div>{formatDate(selected.startDate)} – {formatDate(selected.endDate)}</div><div>{inclusiveDays(selected)} Tage</div></div></div></div><div className="flex gap-2"><OpsButton disabled={!permissions.canEdit} onClick={() => setDialog({ open: true, event: selected })}><Pencil className="mr-2 inline h-4 w-4" />Bearbeiten</OpsButton><OpsButton className="text-[var(--ops-error)]" disabled={!permissions.canDelete} onClick={() => setDeleting(selected)}><Trash2 className="mr-2 inline h-4 w-4"/>Löschen</OpsButton></div></div>
    ...
    <Dialog open={Boolean(deleting)} onClose={() => setDeleting(null)}><DialogTitle>Event löschen?</DialogTitle><DialogContent>Das Event <b>{competitionDisplayName(deleting?.discipline)}</b> wird dauerhaft entfernt.</DialogContent><DialogActions><Button onClick={() => setDeleting(null)}>Abbrechen</Button><Button color="error" variant="contained" onClick={async () => { if (deleting) { await api.deleteEvent(deleting.id); setDeleting(null); await load(); } }}>Löschen</Button></DialogActions></Dialog>
  </SplitPageLayout>;
}
