...
export function EventsManagement() {
  ...
  return <SplitPageLayout>
    ...
    <Dialog open={Boolean(deleting)} onClose={() => setDeleting(null)}><DialogTitle>Event löschen?</DialogTitle><DialogContent>Das Event <b>{deleting?.discipline}</b> wird dauerhaft entfernt.</DialogContent><DialogActions><Button onClick={() => setDeleting(null)}>Abbrechen</Button><Button color="error" variant="contained" onClick={async () => { if (deleting) { await api.deleteEvent(deleting.id); setDeleting(null); await load(); } }}>Löschen</Button></DialogActions></Dialog>
  </SplitPageLayout>;
}
