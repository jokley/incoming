...
export function EventsManagement() {
  ...
  const timelineRows: TimelineRowData[] = events.map(event => ({ id: event.id, title: event.discipline, subtitle: `${inclusiveDays(event)} Tage`, segments: [{ id: `event-${event.id}`, start: event.startDate, end: event.endDate, label: event.discipline, color: event.id === selectedId ? 'var(--ops-primary-emphasis)' : 'var(--ops-info)', tooltipData: { title: event.discipline, description: 'Eventzeitraum ohne Hotelbelegung' } }] }));
