        ...
        <ContentCard surface="raised" className="min-h-0 flex-1 overflow-hidden">
          <div ref={detailScrollRef} className="h-full overflow-y-auto">
            ...
            <div className="space-y-4 p-5">
              ...
              <NextAction session={selected} preview={preview} confirming={confirming} onApprove={approve} onImport={confirm}/>
...
function NewSessionWorkspace({files,preview,loading,confirming,error,success,onFiles,onPreview,onConfirm,onCancel}:{files:File[];preview:FisImportPreview|null;loading:boolean;confirming:boolean;error:string|null;success:string|null;onFiles:(files:FileList|null)=>void;onPreview:()=>void;onConfirm:()=>void;onCancel:()=>void}) { return <>
  <div className="sticky top-0 z-10 flex flex-wrap items-start justify-between gap-4 border-b border-[var(--ops-divider)] bg-[var(--ops-surface-raised)] p-5"><div><SectionHeader title="Importprüfung"/><h2 className="mt-2 text-2xl font-extrabold">Neue Importsession</h2><p className="text-sm text-[var(--ops-text-muted)]">FIS-Dateien hochladen und vor dem Import gemeinsam prüfen.</p></div><div className="flex gap-2"><OpsButton onClick={onPreview} disabled={files.length<2||loading}>{loading?<Loader2 className="mr-2 inline h-4 w-4 animate-spin"/>:<FileCheck2 className="mr-2 inline h-4 w-4"/>}Dateien prüfen</OpsButton><OpsButton onClick={onConfirm} disabled={true} className="border-[var(--ops-tone-success-border)] bg-[var(--ops-tone-success-surface)]"><CheckCircle className="mr-2 inline h-4 w-4"/>Importieren</OpsButton></div></div>
...
function visibleWorkflow(session: ImportSession | null): WorkflowStep[] {
  ...
  return [
    ...
    { id: 'import', label: 'Importieren', complete: imported, current: status === 'APPROVED' || imported },
  ];
}
...
function NextAction({session,preview,confirming,onApprove,onImport}:{session:ImportSession;preview:FisImportPreview|null;confirming:boolean;onApprove:()=>void;onImport:()=>void}) {
  ...
  if(session.status==='IMPORTED') return <InfoPanel tone="success" title="Import abgeschlossen">Diese Importsession wurde vollständig importiert.</InfoPanel>;
  if(pending) return <div className="flex items-center justify-between gap-4 rounded-xl border border-[var(--ops-tone-warning-border)] bg-[var(--ops-tone-warning-surface)] px-4 py-3"><div className="flex items-center gap-3"><AlertTriangle className="h-5 w-5 shrink-0 text-[var(--ops-warning)]"/><div><div className="font-extrabold">Nächster Schritt: Entscheidungen bearbeiten</div><p className="text-sm text-[var(--ops-text-muted)]">{pending} {pending===1?'offene Entscheidung muss':'offene Entscheidungen müssen'} vor der Freigabe abgeschlossen werden.</p></div></div><StatusChip tone="warning">{pending} offen</StatusChip></div>;
  if(session.status==='APPROVED') return <div className="flex items-center justify-between gap-4 rounded-xl border border-[var(--ops-tone-success-border)] bg-[var(--ops-tone-success-surface)] px-4 py-3"><div><div className="font-extrabold">Nächster Schritt: Importieren</div><p className="text-sm text-[var(--ops-text-muted)]">Die Importsession ist freigegeben und kann jetzt importiert werden.</p></div><OpsButton onClick={onImport} disabled={confirming} className="border-[var(--ops-tone-success-border)] bg-[var(--ops-tone-success-surface)]"><CheckCircle className="mr-2 inline h-4 w-4"/>Importieren</OpsButton></div>;
  return <div className="flex items-center justify-between gap-4 rounded-xl border border-[var(--ops-tone-primary-border)] bg-[var(--ops-tone-primary-surface)] px-4 py-3"><div className="flex items-center gap-3">{canApprove?<CheckCircle className="h-5 w-5 text-[var(--ops-primary)]"/>:<LockKeyhole className="h-5 w-5 text-[var(--ops-text-subtle)]"/>}<div><div className="font-extrabold">Nächster Schritt: Freigeben</div><p className="text-sm text-[var(--ops-text-muted)]">{canApprove?'Alle Entscheidungen sind abgeschlossen. Prüfen und freigeben.':'Die Freigabe ist verfügbar, sobald Prüfung und Entscheidungen abgeschlossen sind.'}</p></div></div><OpsButton onClick={onApprove} disabled={!canApprove}>Freigeben</OpsButton></div>;
}
...
function TaskList({session,onOpen}:{session:ImportSession;onOpen:(task:OperationsTask)=>void}) { const pending=session.approvals.map(approval=>buildOperationsTask(session,approval)).filter(task=>task.approval.decision==='PENDING'); if(!pending.length)return null; return <ContentCard surface="elevated" className="border-[var(--ops-tone-warning-border)] p-4"><SectionHeader title="Entscheidungen bearbeiten" subtitle={`${pending.length} ${pending.length===1?'offene Entscheidung':'offene Entscheidungen'} · der Reihe nach abschließen`} actions={<StatusChip tone="warning">Handlungsbedarf</StatusChip>}/><div className="mt-3 space-y-2">{pending.map((task,index)=><OperationsTaskRow key={task.approval.id} task={task} onOpen={()=>onOpen(task)} primary={index===0}/>)}</div></ContentCard>; }
