      ...
      <nav className="z-40 shrink-0 border-b border-[var(--ops-border)] bg-[var(--ops-surface)] shadow-[var(--ops-shadow-xs)]">
        <div className="mx-auto w-full max-w-[1920px] px-3 sm:px-5 lg:px-6">
          ...
          <div className="flex gap-1 overflow-x-auto pb-2 lg:hidden">
            ...
            {permissions.isAdmin && <NavLink to="/administration/test-data" className={({ isActive }) => `inline-flex h-9 shrink-0 items-center rounded-xl px-3 text-xs font-medium ${isActive ? 'bg-[var(--ops-tone-primary-surface)] text-[var(--ops-primary)]' : 'text-[var(--ops-text-muted)]'}`}><Settings className="mr-1.5 h-4 w-4" />Administration</NavLink>}
          </div>
        </div>
      </nav>
