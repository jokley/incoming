...
export type OperationalHotelFilter = 'attention' | 'all' | 'under75' | 'over75' | 'over90' | 'full' | 'freeSingle' | 'freeDouble';
...
export const OPERATIONAL_HOTEL_FILTERS:ReadonlyArray<{value:OperationalHotelFilter;label:string}> = [
  {value:'attention',label:'Handlungsbedarf'}, {value:'all',label:'Alle'}, {value:'under75',label:'< 75 %'},
  {value:'over75',label:'≥ 75 %'}, {value:'over90',label:'> 90 %'}, {value:'full',label:'Voll'},
  {value:'freeSingle',label:'Freie EZ'}, {value:'freeDouble',label:'Freie DZ'},
];

export const hasHotelAttention=(state:OperationalHotelState)=>state.totalCapacity===0||state.occupancy>=75||state.freeCapacity<=2;
export function matchesOperationalHotelFilter(state:OperationalHotelState,filter:OperationalHotelFilter){if(filter==='all')return true;if(filter==='attention')return hasHotelAttention(state);if(filter==='under75')return state.occupancy<75;if(filter==='over75')return state.occupancy>=75;if(filter==='over90')return state.occupancy>90;if(filter==='full')return state.totalCapacity>0&&state.freeCapacity===0;if(filter==='freeSingle')return state.hasFreeSingle;return state.hasFreeDouble;}
export function compareOperationalHotels(a:OperationalHotelState,b:OperationalHotelState){const rank=(state:OperationalHotelState)=>state.totalCapacity===0?0:state.freeCapacity===0?1:state.occupancy>90||state.freeCapacity<=2?2:state.occupancy>=75?3:4;return rank(a)-rank(b)||a.name.localeCompare(b.name,'de');}
