...
export type OperationalHotelFilter = 'all' | 'under75' | 'over75' | 'full' | 'freeSingle' | 'freeDouble';
...
export function matchesOperationalHotelFilter(state:OperationalHotelState,filter:OperationalHotelFilter){if(filter==='all')return true;if(filter==='under75')return state.occupancy<75;if(filter==='over75')return state.occupancy>=75;if(filter==='full')return state.totalCapacity>0&&state.freeCapacity===0;if(filter==='freeSingle')return state.hasFreeSingle;return state.hasFreeDouble;}
