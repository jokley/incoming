...
export function OperationsDecisionDialog({ task, saving, onClose, onSave }: {
  ...
  return <Dialog open={Boolean(task)} onClose={saving ? undefined : onClose} fullWidth maxWidth="md" slotProps={{ paper: { sx: { maxHeight: 'calc(100dvh - 32px)', display: 'flex', overflow: 'hidden' } } }}>
    {task && <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-[var(--ops-surface)] text-[var(--ops-text)]">
      <DialogHeader title={taskTitle(task) || 'Entscheidung bearbeiten'} subtitle="Von oben nach unten bearbeiten · anschließend speichern" />
      <DialogContent dividers className="min-h-0 flex-1 space-y-4 overflow-y-auto">
        ...
        <section><SectionHeader title="Dokumentation" subtitle="Rücksprache und fachliche Notiz festhalten"/><div className="mt-2 grid gap-3 rounded-lg border border-[var(--ops-border)] bg-[var(--ops-surface-elevated)] p-3 sm:grid-cols-2">
          ...
          <Field label="Kommunikationsweg"><select value={method} onChange={e=>setMethod(e.target.value as 'EMAIL'|'PHONE')} className="field"><option value="EMAIL">E-Mail</option><option value="PHONE">Telefon</option></select></Field>
          ...
          <Field label="E-Mail-Betreff (optional)"><input value={subject} onChange={e=>setSubject(e.target.value)} className="field"/></Field>
          ...
          <label htmlFor="decision-comment" className="block text-sm font-bold sm:col-span-2">Dispositionsnotiz (optional)<textarea id="decision-comment" value={comment} onChange={event=>setComment(event.target.value)} rows={3} placeholder="Telefonnotiz, E-Mail oder ergänzende Information …" className="mt-1 w-full resize-y rounded-lg border border-[var(--ops-border)] bg-[var(--ops-background)] p-2.5 text-sm text-[var(--ops-text)] outline-none placeholder:text-[var(--ops-text-subtle)] focus:border-[var(--ops-primary)]" /></label>
        </div></section>
