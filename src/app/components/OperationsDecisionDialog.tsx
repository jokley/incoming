  ...
  return <Dialog open={Boolean(task)} onClose={saving ? undefined : onClose} fullWidth maxWidth="md" slotProps={{ paper: { sx: { maxHeight: 'calc(100dvh - 32px)', display: 'flex', overflow: 'hidden' } } }}>
    {task && <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-[var(--ops-surface)] text-[var(--ops-text)]">
      ...
      <DialogContent dividers className="min-h-0 flex-1 space-y-6 overflow-y-auto">
        ...
        <section><label htmlFor="decision-comment" className="block"><SectionHeader title="Dispositionsnotiz" subtitle="Optional: Telefonnotiz, Mail oder ergänzende Information" /></label><textarea id="decision-comment" value={comment} onChange={event=>setComment(event.target.value)} rows={4} placeholder="Dispositionsnotiz …" className="mt-3 w-full resize-y rounded-xl border border-[var(--ops-border)] bg-[var(--ops-background)] p-3 text-sm text-[var(--ops-text)] outline-none placeholder:text-[var(--ops-text-subtle)] focus:border-[var(--ops-primary)]" /></section>
