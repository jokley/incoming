...
import { Alert, Box, Button, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, IconButton, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Tooltip, Typography } from '@mui/material';
import { ArrowDownToLine, Database, HardDrive, RefreshCw, RotateCcw, Upload } from 'lucide-react';
import { api, DatabaseBackup, DatabaseStatus } from '../services/api';
...
const categories = [
  { key: 'automatic', title: 'Automatische Backups' },
  { key: 'manual', title: 'Manuelle Backups' },
  { key: 'pre-restore', title: 'Vor Wiederherstellung' },
] as const;
...
export function DatabaseBackups({ embedded = false }: { embedded?: boolean }) {
  ...
  const download = (row: DatabaseBackup) => { if (!row.localFile) { if (!row.category) return; void api.downloadDatabaseBackup(row.filename, row.category).catch(() => setNotice({ severity: 'error', text: 'Das Backup konnte nicht heruntergeladen werden.' })); return; } const url = URL.createObjectURL(row.localFile); const anchor = document.createElement('a'); anchor.href = url; anchor.download = row.filename; anchor.click(); URL.revokeObjectURL(url); };
  const cards = [['PostgreSQL Status', status ? 'Bereit' : '–'], ['PostgreSQL Version', status?.postgresVersion ?? '–'], ['Alembic Version', status?.alembicVersion ?? '–'], ['Datenbankgröße', bytes(status?.databaseSize)], ['Letztes erfolgreiches Backup', date(status?.lastBackup?.created)], ['Größe des letzten Backups', bytes(status?.backupSize)]];
  return <Box sx={{ height: '100%', overflowY: 'auto', pb: 3 }}>
    ...
    {categories.map(category => { const rows = backups.filter(backup => backup.category === category.key); return <Paper key={category.key} variant="outlined" sx={{ mb: 2, borderRadius: 3, overflow: 'hidden' }}><Box sx={{ p: 2.5, pb: 1 }}><Typography variant="h6" fontWeight={700}>{category.title}</Typography></Box><TableContainer><Table><TableHead><TableRow><TableCell>Datum</TableCell><TableCell>Größe</TableCell><TableCell align="right">Aktionen</TableCell></TableRow></TableHead><TableBody>{rows.map(row => <TableRow key={`${category.key}-${row.filename}`} hover><TableCell><Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}><Database size={18} /><Typography variant="body2" fontWeight={650}>{date(row.modified)}</Typography></Box></TableCell><TableCell>{bytes(row.size)}</TableCell><TableCell align="right"><Tooltip title="Herunterladen"><IconButton aria-label={`${row.filename} herunterladen`} onClick={() => download(row)}><ArrowDownToLine size={19} /></IconButton></Tooltip><Tooltip title="Wiederherstellen"><IconButton aria-label={`${row.filename} wiederherstellen`} onClick={() => setSelected(row)}><RotateCcw size={19} /></IconButton></Tooltip></TableCell></TableRow>)}{!rows.length && busy !== 'load' && <TableRow><TableCell colSpan={3} align="center" sx={{ py: 3, color: 'text.secondary' }}>Noch kein Backup vorhanden.</TableCell></TableRow>}</TableBody></Table></TableContainer></Paper>; })}
    <Dialog open={Boolean(selected)} onClose={() => busy !== 'restore' && setSelected(null)} maxWidth="sm" fullWidth><DialogTitle>Backup wiederherstellen</DialogTitle><DialogContent><Typography>Vor der Wiederherstellung wird automatisch ein Sicherheitsbackup der aktuellen Datenbank erstellt.</Typography><Typography sx={{ mt: 2 }}>Die Anwendung ist während der Wiederherstellung kurzzeitig nicht verfügbar.</Typography>{busy === 'restore' && <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mt: 3 }}><CircularProgress size={22} /><Typography color="text.secondary">Backup wird wiederhergestellt …</Typography></Box>}</DialogContent><DialogActions><Button disabled={busy === 'restore'} onClick={() => setSelected(null)}>Abbrechen</Button><Button variant="contained" disabled={busy === 'restore'} startIcon={<RefreshCw size={18} />} onClick={restore}>Wiederherstellen</Button></DialogActions></Dialog>
  </Box>;
}
