...
function HotelContactsView({ hotels, navigate }: { hotels: Hotel[]; navigate: ReturnType<typeof useNavigate> }) {
  ...
  return <div className="flex min-h-0 min-w-0 flex-col gap-3"><ContentCard className="p-2.5"><Toolbar className="border-0 bg-transparent p-0 shadow-none"><label className="flex min-w-[15rem] flex-1 items-center gap-2 rounded-lg border border-[var(--ops-border)] bg-[var(--ops-surface-elevated)] px-2.5 py-1.5"><Search size={15}/><input aria-label="Hotelkontakte durchsuchen" className="w-full bg-transparent text-xs outline-none" placeholder="Hotel, Ort oder Kontakt suchen" value={search} onChange={event => setSearch(event.target.value)}/></label></Toolbar></ContentCard>
    <ContentCard className="flex min-h-0 flex-1 flex-col overflow-hidden"><div className="flex items-center justify-between border-b border-[var(--ops-divider)] px-3 py-1.5"><div><h2 className="text-xs font-extrabold">Hotelübersicht</h2><p className="text-[10px] text-[var(--ops-text-muted)]">{rows.length} Hotels · Kontakt- und Stammdaten</p></div><OpsButton className="px-2.5 py-1 text-[10px]" onClick={()=>exportHotelOverviewExcel(rows)} disabled={!rows.length}><FileSpreadsheet className="mr-1.5 inline" size={13}/>Excel</OpsButton></div>
...
type ViewKind = ListKind | 'hotelContacts';
...
export function Lists() {
  ...
  const switchKind = (next: ViewKind) => { setKind(next); setFilters(current => ({ ...current, selection: '' })); };
  ...
  return <SplitPageLayout density="compact">
    ...
    {!data && !error ? <LoadingState label="Live-Listen werden geladen…" /> : data && <>
      ...
      <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[14rem_minmax(0,1fr)]">
        <ContentCard className="h-fit overflow-hidden"><div className="border-b border-[var(--ops-divider)] px-3 py-2 text-[10px] font-extrabold uppercase tracking-[.14em] text-[var(--ops-text-subtle)]">Listenansicht</div><nav className="space-y-0.5 p-1.5" aria-label="Listen">{([{ id: 'hotels', label: 'Personen je Hotel', icon: Building2 }, { id: 'nations', label: 'Personen je Nation', icon: Users }, { id: 'hotelContacts', label: 'Hotelübersicht', icon: ContactRound }, { id: 'contingents', label: 'Hotelkontingente', icon: Boxes }] as const).map(({ id, label, icon: Icon }) => <button key={id} onClick={() => switchKind(id)} className={`flex w-full items-center justify-between rounded-lg px-2.5 py-2 text-left text-xs font-bold transition ${kind === id ? 'bg-[var(--ops-tone-primary-surface)] text-[var(--ops-primary)]' : 'text-[var(--ops-text-muted)] hover:bg-[var(--ops-surface-elevated)] hover:text-[var(--ops-text)]'}`}><span className="flex items-center gap-2"><Icon size={15}/>{label}</span></button>)}</nav></ContentCard>
