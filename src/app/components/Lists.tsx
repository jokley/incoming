...
function FragmentRows({ child, kind, navigate }: { child: Child; kind: ListKind; navigate: ReturnType<typeof useNavigate> }) {
  ...
  return <>
    <tr className="border-b border-[var(--ops-divider)] bg-[var(--ops-surface-raised)] text-[var(--ops-text-muted)]">
      <td colSpan={columns.length + (kind === 'nations' ? 1 : 0)} className="px-3 py-1.5 font-bold">
    ...
    {child.rows.map(row => <tr key={row.id} className="border-b border-[var(--ops-divider)] text-[11px] leading-4 hover:bg-[var(--ops-surface-elevated)]">
      <td className="px-2.5 py-1"><button className="font-semibold text-[var(--ops-primary)] hover:underline" onClick={() => navigate(assignmentWorkspaceHref({ bookingId: row.bookingId, hotelId: row.hotelId, personId: row.id }))}>{row.room}</button></td>
      <td className="px-2.5 py-1"><span className={`inline-flex rounded-full border px-1 py-0 text-[9px] font-bold leading-3.5 ${roomTone(row.roomType)}`}>{roomCode(row.roomType)}</span></td>
      <td className="whitespace-nowrap px-2.5 py-1"><button className="font-semibold text-[var(--ops-primary)] hover:underline" onClick={() => navigate(`/athletes?athleteId=${row.id}`)}>{row.name}</button></td>
      {[row.nation, row.discipline, row.role].map((value, index) => <td key={index} className="px-2.5 py-1">{value}</td>)}
      <td className="px-2.5 py-1 tabular-nums">{formatDate(row.arrival)}</td>
      <td className="px-2.5 py-1 tabular-nums">{formatDate(row.departure)}</td>
      {[row.firstMeal, row.lastMeal, row.specialMeal, row.lateCheckout, row.surcharge, row.roommate].map((value, index) => <td key={index} className="px-2.5 py-1">{value}</td>)}
      <td className="px-2.5 py-1 text-center">{row.remark !== '—' && <Tooltip><TooltipTrigger asChild><button type="button" aria-label={`Bemerkung zu ${row.name}`} className="inline-flex rounded p-0.5 text-[var(--ops-text-muted)] hover:bg-[var(--ops-tone-primary-surface)] hover:text-[var(--ops-primary)]"><NotebookPen size={13}/></button></TooltipTrigger><TooltipContent side="top" sideOffset={5} className="max-w-80 whitespace-pre-wrap break-words bg-[var(--ops-text)] text-left leading-relaxed text-[var(--ops-surface)]">{row.remark}</TooltipContent></Tooltip>}</td>
      {kind === 'nations' && <td className="px-2.5 py-1"><button className="font-semibold text-[var(--ops-primary)] hover:underline" onClick={() => navigate(`/hotels?hotelId=${row.hotelId}`)}>{row.hotel}</button></td>}
    </tr>)}
  </>;
}
    ...
    {!data && !error ? <LoadingState label="Live-Listen werden geladen…" /> : data && <>
      ...
      <div className="grid gap-4 lg:grid-cols-[14rem_minmax(0,1fr)]">
        ...
        <div className="min-w-0 space-y-3">
          ...
          <ContentCard className="overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--ops-divider)] px-3 py-2"><div><h2 className="text-sm font-extrabold">{kind === 'hotels' ? 'Hotel-Liste' : 'Nationen-Liste'}</h2><p className="text-[11px] text-[var(--ops-text-muted)]">{rows.length} Personen · aktuell gefilterte Live-Ansicht</p></div><OpsButton className="px-2.5 py-1.5 text-xs" onClick={() => exportExcel(rows, kind)} disabled={!rows.length}><FileSpreadsheet className="mr-1.5 inline" size={14}/>Excel</OpsButton></div>
              ...
              <table className="w-full min-w-[1320px] border-collapse text-left text-xs">
                <thead className="sticky top-0 z-30 bg-[var(--ops-surface)] shadow-sm">
                  <tr className="border-y border-[var(--ops-divider)] text-[10px] uppercase tracking-wider text-[var(--ops-text-subtle)]">
                    {[...columns, ...(kind === 'nations' ? ['Hotel'] : [])].map(label => <th key={label} className="whitespace-nowrap px-2.5 py-1.5 font-extrabold">{label}</th>)}
                  </tr>
                </thead>
                {groups.map(group => {
                  ...
                  return <tbody key={group.label}>
                    <tr className="border-y border-[var(--ops-divider)] bg-[var(--ops-surface-elevated)]">
                      <td colSpan={columns.length + (kind === 'nations' ? 1 : 0)} className="px-3 py-1.5">
