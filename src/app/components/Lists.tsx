...
function ContingentDataRow({ row, alternate, navigate }: { row: ContingentRow; alternate: boolean; navigate: ReturnType<typeof useNavigate> }) {
  ...
  return <tr className={`${alternate ? 'bg-[var(--ops-surface-elevated)]/45' : 'bg-[var(--ops-surface)]'} border-b border-[var(--ops-divider)] text-[11px] leading-4 hover:bg-[var(--ops-surface-raised)]`}>
    ...
    <td className="px-2 py-1.5 text-center">{row.comment !== '—' && <Tooltip><TooltipTrigger asChild><button type="button" aria-label={`Kontingent-Kommentar zu ${row.hotel}`} className="inline-flex rounded p-1 text-[var(--ops-text-muted)] hover:text-[var(--ops-primary)]"><NotebookPen size={13}/></button></TooltipTrigger><TooltipContent className="max-w-80 whitespace-pre-wrap text-xs">{row.comment}</TooltipContent></Tooltip>}</td>
  </tr>;
}
...
function HotelContactsView({ hotels, navigate }: { hotels: Hotel[]; navigate: ReturnType<typeof useNavigate> }) {
  ...
  return <div className="flex min-h-0 min-w-0 flex-col gap-3"><ContentCard className="p-2.5"><Toolbar className="border-0 bg-transparent p-0 shadow-none"><label className="flex min-w-[15rem] flex-1 items-center gap-2 rounded-lg border border-[var(--ops-border)] bg-[var(--ops-surface-elevated)] px-2.5 py-1.5"><Search size={15}/><input aria-label="Hotelkontakte durchsuchen" className="w-full bg-transparent text-xs outline-none" placeholder="Hotel, Ort oder Kontakt suchen" value={search} onChange={event => setSearch(event.target.value)}/></label></Toolbar></ContentCard>
    <ContentCard className="flex min-h-0 flex-1 flex-col overflow-hidden"><div className="border-b border-[var(--ops-divider)] px-3 py-1.5"><h2 className="text-xs font-extrabold">Hotelkontakte</h2><p className="text-[10px] text-[var(--ops-text-muted)]">{rows.length} Hotels · Stammdaten ohne Kontingente</p></div>
      {!rows.length ? <div className="p-5"><EmptyState title="Keine Hotelkontakte" description="Passen Sie die Suche an."/></div> : <div className="min-h-0 flex-1 overflow-auto"><table className="w-full min-w-[800px] border-collapse text-left text-xs"><thead className="sticky top-0 z-30 bg-[var(--ops-surface)] shadow-sm"><tr className="border-y border-[var(--ops-divider)] text-[10px] uppercase tracking-wider text-[var(--ops-text-subtle)]">{([['hotel','Hotel'],['location','Ort'],['contactPerson','Ansprechpartner'],['phone','Telefon'],['email','E-Mail'],['comment','Hotel-Kommentar']] as const).map(([key,label]) => <SortableTableHead key={key} column={key} label={label} sort={sort} onSort={toggleSort} align={key === 'comment' ? 'center' : 'left'}/>)}</tr></thead><tbody>{rows.map((row,index)=><tr key={row.id} className={`${index%2?'bg-[var(--ops-surface-elevated)]/45':'bg-[var(--ops-surface)]'} border-b border-[var(--ops-divider)] text-[11px] hover:bg-[var(--ops-surface-raised)]`}><td className="px-2 py-2"><button className="font-bold hover:text-[var(--ops-primary)] hover:underline" onClick={()=>navigate(`/hotels?hotelId=${row.id}`)}>{row.hotel}</button></td><td className="px-2 py-2">{row.location}</td><td className="px-2 py-2">{row.contactPerson}</td><td className="px-2 py-2">{row.phone}</td><td className="px-2 py-2">{row.email}</td><td className="px-2 py-2 text-center">{row.comment !== '—' && <Tooltip><TooltipTrigger asChild><button type="button" aria-label={`Hotel-Kommentar zu ${row.hotel}`} className="inline-flex rounded p-1 text-[var(--ops-text-muted)] hover:text-[var(--ops-primary)]"><NotebookPen size={13}/></button></TooltipTrigger><TooltipContent className="max-w-80 whitespace-pre-wrap text-xs">{row.comment}</TooltipContent></Tooltip>}</td></tr>)}</tbody></table></div>}
    </ContentCard></div>;
}
  ...
  return <SplitPageLayout density="compact">
    ...
    {!data && !error ? <LoadingState label="Live-Listen werden geladen…" /> : data && <>
      ...
      <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[14rem_minmax(0,1fr)]">
        <ContentCard className="h-fit overflow-hidden"><div className="border-b border-[var(--ops-divider)] px-3 py-2 text-[10px] font-extrabold uppercase tracking-[.14em] text-[var(--ops-text-subtle)]">Gruppierung</div><nav className="space-y-0.5 p-1.5" aria-label="Listen">{([{ id: 'hotels', label: 'Hotels', icon: Building2 }, { id: 'nations', label: 'Nationen', icon: Users }, { id: 'contingents', label: 'Kontingente', icon: Boxes }, { id: 'hotelContacts', label: 'Hotelkontakte', icon: ContactRound }] as const).map(({ id, label, icon: Icon }) => <button key={id} onClick={() => switchKind(id)} className={`flex w-full items-center justify-between rounded-lg px-2.5 py-2 text-left text-xs font-bold transition ${kind === id ? 'bg-[var(--ops-tone-primary-surface)] text-[var(--ops-primary)]' : 'text-[var(--ops-text-muted)] hover:bg-[var(--ops-surface-elevated)] hover:text-[var(--ops-text)]'}`}><span className="flex items-center gap-2"><Icon size={15}/>{label}</span></button>)}</nav></ContentCard>
