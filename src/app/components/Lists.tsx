...
function FragmentRows({ child, kind, navigate }: { child: Child; kind: ListKind; navigate: ReturnType<typeof useNavigate> }) {
  ...
  return <>
    ...
    {child.rows.map(row => <tr key={row.id} className="border-b border-[var(--ops-divider)] hover:bg-[var(--ops-surface-elevated)]">
      <td className="px-2.5 py-1.5"><button className="font-bold text-[var(--ops-primary)] hover:underline" onClick={() => navigate(assignmentWorkspaceHref({ bookingId: row.bookingId, hotelId: row.hotelId, personId: row.id }))}>{row.room}</button></td>
      <td className="px-2.5 py-1.5"><span className={`inline-flex rounded-full border px-1.5 py-px text-[10px] font-extrabold ${roomTone(row.roomType)}`}>{roomCode(row.roomType)}</span></td>
      <td className="whitespace-nowrap px-2.5 py-1.5"><button className="font-bold text-[var(--ops-primary)] hover:underline" onClick={() => navigate(`/athletes?athleteId=${row.id}`)}>{row.name}</button></td>
      {[row.nation, row.discipline, row.role].map((value, index) => <td key={index} className="px-2.5 py-1.5">{value}</td>)}
      <td className="px-2.5 py-1.5 tabular-nums">{formatDate(row.arrival)}</td>
      <td className="px-2.5 py-1.5 tabular-nums">{formatDate(row.departure)}</td>
      {[row.firstMeal, row.lastMeal, row.specialMeal, row.lateCheckout, row.surcharge, row.roommate].map((value, index) => <td key={index} className="px-2.5 py-1.5">{value}</td>)}
      <td className="px-2.5 py-1.5 text-center">{row.remark !== '—' && <Tooltip><TooltipTrigger asChild><button type="button" aria-label={`Bemerkung zu ${row.name}`} className="inline-flex rounded p-0.5 text-[var(--ops-text-muted)] hover:bg-[var(--ops-tone-primary-surface)] hover:text-[var(--ops-primary)]"><NotebookPen size={14}/></button></TooltipTrigger><TooltipContent side="top" sideOffset={5} className="max-w-80 whitespace-pre-wrap break-words bg-[var(--ops-text)] text-left leading-relaxed text-[var(--ops-surface)]">{row.remark}</TooltipContent></Tooltip>}</td>
      {kind === 'nations' && <td className="px-2.5 py-1.5"><button className="font-bold text-[var(--ops-primary)] hover:underline" onClick={() => navigate(`/hotels?hotelId=${row.hotelId}`)}>{row.hotel}</button></td>}
    </tr>)}
  </>;
}
