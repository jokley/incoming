...
import { ArchiveRestore, Database, Settings, ShieldCheck, SlidersHorizontal, Users } from 'lucide-react';
import { AdministrationTestData } from './AdministrationTestData';
import { DatabaseBackups } from './DatabaseBackups';
import { ContentCard, EmptyState, PageHeader, SplitPageLayout, StatusChip } from '../design-system';

const sections = [
  { id: 'database', label: 'Datenbank', icon: Database },
  { id: 'backups', label: 'Backups', icon: ArchiveRestore },
  { id: 'users', label: 'Benutzer', icon: Users },
  { id: 'reference', label: 'Referenzdaten', icon: SlidersHorizontal },
...
export function Administration() {
  ...
  return <SplitPageLayout><PageHeader eyebrow="Systemverwaltung" title="Administration" subtitle="Datenbank, Betriebsdaten und Wartung zentral verwalten." meta={<StatusChip tone="neutral"><ShieldCheck className="mr-1 inline" size={13}/>Geschützter Bereich</StatusChip>}/>
    <div className="flex flex-col gap-5 xl:min-h-0 xl:flex-1 xl:flex-row">
      ...
      <ContentCard surface="raised" className="overflow-hidden xl:min-h-0 xl:flex-1"><div className="border-b border-[var(--ops-divider)] p-5"><h2 className="text-xl font-extrabold">{sections.find(item => item.id === active)?.label}</h2><p className="mt-1 text-sm text-[var(--ops-text-muted)]">Detailinformationen und Aktionen</p></div><div className="p-5 xl:h-[calc(100%-5rem)] xl:overflow-y-auto">{active === 'database' && <AdminSection title="Datenverwaltung" description="Testdaten, Athleten, Zuweisungen und Workflows verwalten."><AdministrationTestData embedded/></AdminSection>}{active === 'backups' && <AdminSection title="Datenbank & Backups" description="PostgreSQL-Status, Sicherungen und Wiederherstellung."><DatabaseBackups embedded/></AdminSection>}{active === 'users' && <EmptyState title="Benutzerverwaltung" description="Benutzer und Berechtigungen werden über die bestehende Authentifizierung verwaltet."/>}{active === 'reference' && <EmptyState title="Referenzdaten" description="Referenzdaten werden in den jeweiligen Stammdatenbereichen gepflegt."/>}{active === 'settings' && <EmptyState title="Einstellungen" description="Derzeit sind keine zusätzlichen Wartungsaktionen erforderlich."/>}</div></ContentCard>
    </div>
  </SplitPageLayout>;
}
